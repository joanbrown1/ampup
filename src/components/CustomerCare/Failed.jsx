...
const Failed = () => {
    ...
    const getTransactions = async () => {
       ...
       setMessage("Loading")
        ...
        try {
           ...
           let response = await fetch("https://api.powerkiosk.ng/api/faileds", { 
             method: "GET",
             headers: {
                "Accept": "*/*"
             }
    ...
    useEffect(() => {
      ...
      useEffect(() => {
        setCurrentPage(1); // Reset to first page on transaction change
    }, [transaction]);
        ...
        try {
            ...
            if (data.code === "000") {
                ...
                setTransaction(prevTransaction =>
                    prevTransaction.map(user => {
                        // console.log(`Checking user ppid: ${user.ppid} against: ${ppid}`);
                        if (user.ppid === ppid) {
                            console.log(data.Token)
                        }
                        return user.ppid === ppid ? { ...user, token: data.Token } : user;
                    })
                );
