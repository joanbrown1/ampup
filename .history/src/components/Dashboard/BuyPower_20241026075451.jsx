...
const BuyPower = () => {
    ...
    const [formData, setFormData] = useState({
      ...
      useEffect(() => {
        ...
        const getAnnouncement = () => {
          ...
          .then(data => {
            setAnnouncement(data); // Set the announcements data in state
          })
          .catch(error => {
      ...
      const activeDiscos = discos.filter(disco => disco.active === 1);
      const inactiveDiscos = discos.filter(disco => disco.active === 0);
    ...
    useEffect(() => {
      ...
      const checkMeter = async (e) =>{
        ...
        if (formData.amount < 600 ) newErrors.amount = "Amount must be more than 1200.";
    ...
    const saveFailedTransaction  = async (powerdata)=>{
        ...
        try {
            ...
            let res = await fetch("https://api.powerkiosk.ng/api/failed", { 
                method: "POST",
                body: body,
                headers: {
                "Content-Type": "application/json"
  ...
  return (
    <div>
        <Helmet>
        {/* Google Tag (gtag.js) */}
        <script async src="https://www.googletagmanager.com/gtag/js?id=AW-16737371627"></script>
        <script>
          {`
            window.dataLayer = window.dataLayer || [];
            function gtag(){dataLayer.push(arguments);}
            gtag('js', new Date());
            gtag('config', 'AW-16737371627');
          `}
        </script>
      </Helmet>
    {
        ...
        <div className='lg:col-span-10'>
            <div className="sm:block hidden bg-[#F3F3F3] pt-10 w-full h-full pb-10">
                <div className='ml-6'>
                    ...
                    {announcement.map(announce => (
                        <Alert color="warning" className='my-2 w-full'>
                           {announce.message}
                        </Alert>
                    ))}
                    {inactiveDiscos.length > 0 ?
                        <div className='my-2 lg:w-full  w-[350px] text-center'>
                        <h3 className='font-bold text-secondary text-lg underline'>The DISCO(s) below is(are) experiencing service downtime, please retry after a while:</h3>
                        {inactiveDiscos.map(disco => (
                            <p className='text-md my-1 lg:text-left lg:ml-60 mt-4 lg:font-bold lg:mt-2' key={disco.id}>
                                { disco.name }
                            </p>
                        ))}
                    </div> : ""}
                    <div className='ml-[300px] mt-10'>
                        {
                            ...
                            <div className='flex w-[400px]'>
                                <div onClick={handleReload}>
                                    <p className='text-seconday cursor-pointer italic underline text-red-600' >Reload Token</p>
                                </div>
                                <div onClick={handleShare} className='ml-auto'>
            ...
            <div className="lg:hidden md:hidden bg-[#F3F3F3] w-full h-full flex justify-center ">
                <div className='my-10 pb-10'>
                    ...
                    {announcement.map(announce => (
                        <Alert color="warning" className='my-2 w-[350px]'>
                           {announce.message}
                        </Alert>
                    ))}
                    {inactiveDiscos.length > 0 ?
