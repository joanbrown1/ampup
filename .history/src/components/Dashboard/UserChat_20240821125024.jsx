...
const UserChat = () => {
    ...
    useEffect(() => {
        ...
        const checkConvo = async () => {
            ...
            try {
                const response = await fetch('https://api.powerkiosk.ng/api/conversation/sender', {
                    method: 'POST',
                    headers: {
                ...
                // Check if the response is an array and has at least one conversation
                if (Array.isArray(data) && data.length > 0 && data[0].convo_id) {
                    console.log('Existing conversation found:', data[0].convo_id);
                    fetchMessages(data[0].convo_id);
                } else {
    ...
    const fetchMessages = async (convo_id) => {
        ...
        try {
            const response = await fetch('https://api.powerkiosk.ng/api/messages/convoid', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            if (data.message === 'No messages found for the provided convo id') {
                setMessages([]);
            } else {
                setMessages(data);
            }
    ...
    const sendMessage = async () => {
        ...
        try {
            const response = await fetch('https://api.powerkiosk.ng/api/message', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
    ...
    const startNewConversation = async () => {
        ...
        try {
            const response = await fetch('https://api.powerkiosk.ng/api/conversation', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
