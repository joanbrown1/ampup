    ...
    try {
      ...
      if (response.ok) {
        ...
        setTransaction(data);
         ...
         const totalCommission = data.reduce((acc, transaction) => acc + (transaction.commission || 0), 0);
         setTcommission(totalCommission);

         const totalProfit = data.reduce((acc, transaction) => acc + (transaction.profit || 0), 0);
