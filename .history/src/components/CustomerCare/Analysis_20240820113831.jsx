    ...
    try {
      ...
      if (response.ok) {
        ...
        setTransaction(data);
         ...
         const totalAmountPaid = data.reduce((acc, transaction) => acc + transaction.amount, 0);
         setAmountpaid(totalAmountPaid);

         // Assuming 'cost', 'commission', and 'profit' are part of each transaction object
         const totalCost = data.reduce((acc, transaction) => acc + (transaction.cost || 0), 0);
         setTcost(totalCost);

         const totalCommission = data.reduce((acc, transaction) => acc + (transaction.commision || 0), 0);
         setTcommission(totalCommission);

         const totalProfit = data.reduce((acc, transaction) => acc + (transaction.tpl || 0), 0);
         setTprofit(totalProfit);

         const totalCharge = data.reduce((acc, transaction) => acc + (transaction.charge || 0), 0);
                ...
                <div className='overflow-x-auto'>
                  <div>
                    <div className='flex text-black font-base font-semibold'>
                      {["Email", "Date", "Amount Paid", "Units", "Charge", "VT Pass ID", "Profit Kiosk ID", "Discount Percent", "Discount Amount", "Cost", "Commission Earned", "Total Profit/Loss", "Total Profit Margin"].map(header => (
