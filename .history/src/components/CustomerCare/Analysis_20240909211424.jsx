  ...
  const getTransactions = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const totalProfit = data.reduce((acc, transaction) => acc + parseFloat(transaction.tpl || 0), 0);
                        ...
                        return (
                          <div key={index} onClick={() => handleUserClick(user.email)} style={{ cursor: 'pointer' }}>
                            <div className='flex'>
                              ...
                              <p className='my-4 min-w-[275px]'>{totalProfitLoss.toFixed(2)}</p> {/* Display the calculated tpl */}
                            </div>
                          </div>
                        );
                      })}
