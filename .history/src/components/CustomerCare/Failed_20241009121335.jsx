    ...
    const handleReload = async (ppid) => {
        ...
        try {
            ...
            if (data.code === "000") {
                ...
                // Update only the specific transaction with the new token
                setTransaction((prevTransaction) =>
                    prevTransaction.map((transaction) =>
                        transaction.ppid === ppid
                            ? { ...transaction, token: data.token } // Update token
                            : transaction // Keep others the same
                    )
                );
                                    ...
                                    {currentUsers.map((user, index) => (
                                        <div key={index} style={{ cursor: 'pointer' }}>
                                            <div className='flex'>
                                                ...
                                                <button
                                                    className='my-2 bg-secondary rounded-lg text-white py-2 px-4 min-w-[120px] mx-2 font-bold'
                                                    onClick={() => handleReload(user.ppid)}
                                                    disabled={search === "Loading"} // Disable while loading
                                                >
                                                    {search === "Loading" ? "Reloading..." : "Reload"}
                                                </button>
